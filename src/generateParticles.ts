...
declare const TweenLite: any; // Declare TweenLite as a global variable
declare const Circ: any; // Declare TweenLite as a global variable
...
const generateParticles = (
  ...
  class Circle {
    ...
    constructor(pos: IPoint, rad: number | null, color: string) {
      this.pos = pos || null;
  ...
  function getDistance(p1: ICoordinates, p2: IPoint) {
  ...
  function drawLines(p: IPoint) {
    if (!p.active) return;

    p.closest.forEach((item: { x: number; y: number }) => {
      if (!ctx) {
        return;
      }
      ctx.beginPath();
  ...
  return { mouseMove, scrollCheck, resize };
};
